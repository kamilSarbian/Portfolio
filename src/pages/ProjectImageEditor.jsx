...
export default function ProjectImageEditor({ onGoProjects }) {
  const { t } = useTranslation();
  const stack = t("projects.caseStudies.imageProcessing.stack", { returnObjects: true });
      ...
      <div style={{ width: "min(980px, 100%)" }}>
        <Card>
          ...
          <h1 className="h1" style={{ fontSize: 30 }}>
            {t("projects.caseStudies.imageProcessing.title")}
          </h1>

          <p className="p">{t("imageEditor.desc")}</p>

          <CaseStudySummary caseId="imageProcessing" />
